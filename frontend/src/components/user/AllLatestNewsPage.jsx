      ...
      <div className="max-w-7xl mx-auto flex flex-col md:flex-row gap-8 min-h-screen">
        ...
        <div className={`w-full md:w-[72%] flex flex-col p-8 rounded-3xl border shadow-xl transition-colors duration-300 ${darkMode ? 'bg-gray-900 border-purple-800' : 'bg-white border-purple-100'}` }>
          <h2 className={`text-2xl font-extrabold text-transparent bg-clip-text bg-gradient-to-r from-purple-700 via-pink-600 to-blue-600 mb-6 flex items-center gap-2 tracking-tight drop-shadow-lg ${darkMode ? 'drop-shadow-xl' : ''}` }>
            <i className="fas fa-fire text-pink-500"></i> All Latest News
          </h2>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {newsList.length === 0 ? (
              <p className={`text-sm col-span-full ${darkMode ? 'text-gray-400' : 'text-gray-500'}`}>No latest news found.</p>
            ) : (
              newsList.map((news) => (
                <Link
                  key={news._id}
                ...
                >
                  <div className="h-48 overflow-hidden relative">
                    ...
                    />
                    <span className="absolute top-2 left-2 bg-pink-500 text-white text-xs px-2 py-0.5 rounded-full font-semibold shadow">Latest</span>
                  </div>
                  <div className="p-4 flex-1 flex flex-col">
                    <h3 className={`text-lg font-bold mb-1 group-hover:text-pink-700 transition-colors line-clamp-2 ${darkMode ? 'text-white drop-shadow-lg' : 'text-gray-900'}`}>{news.title}</h3>
                    <p className={`text-sm mb-2 line-clamp-3 ${darkMode ? 'text-gray-300' : 'text-gray-600'}`}>{news.description}</p>
                  </div>
                </Link>
              ))
            )}
